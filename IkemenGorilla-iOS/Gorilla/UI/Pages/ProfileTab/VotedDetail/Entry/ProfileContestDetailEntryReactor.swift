//
//  ProfileContestDetailEntryReactor.swift
//  Gorilla
//
...
final class ProfileContestDetailEntryReactor: Reactor {
    enum Action {
        case refresh
        case load
    }
    ...
    struct State {
        let contest: Contest
        var entryCellReactors: [ProfileContestDetailEntryCellReactor] = []
    ...
    let initialState: ProfileContestDetailEntryReactor.State
    ...
    func reduce(state: State, mutation: Mutation) -> State {
        ...
        case .setEntryCellReactors(let entries):
            state.entryCellReactors = entries.map { ProfileContestDetailEntryCellReactor(entry: $0) }
    ...
    func createContestAnimalDetailReactor(indexPath: IndexPath) -> VotedAnimalDetailReactor {
        return VotedAnimalDetailReactor(provider: provider, entry: currentState.entryCellReactors[indexPath.row].currentState.entry, contestId: currentState.contest.id)
    }
}
