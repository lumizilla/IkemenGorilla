//
//  ProfileContestDetailResultViewController.swift
//  Gorilla
//
...
final class ProfileContestDetailResultViewController: UIViewController, View, ViewConstructor, SegementSlideContentScrollViewDelegate, TransitionPresentable {
    
    struct Reusable {
        static let awardCell = ReusableCell<ProfileContestDetailResultAwardCell>()
        static let resultCell = ReusableCell<ProfileContestDetailResultVoteCell>()
    }
    ...
    private let awardsCollectionView = UICollectionView(frame: .zero, collectionViewLayout: UICollectionViewFlowLayout().then {
        $0.itemSize = ProfileContestDetailResultAwardCell.Const.itemSize
    ...
    private let resultsCollectionView = UICollectionView(frame: .zero, collectionViewLayout: UICollectionViewFlowLayout().then {
        $0.itemSize = ProfileContestDetailResultVoteCell.Const.itemSize
        $0.minimumLineSpacing = 0
    }).then {
    ...
    // MARK: - Bind Method
    func bind(reactor: ProfileContestDetailResultReactor) {
        // Action
        reactor.action.onNext(.load)
        
        awardsCollectionView.rx.itemSelected
            .bind { [weak self] indexPath in
                self?.showVotedAnimalDetailPage(votedAnimalDetailReactor: reactor.createVotedDetailReactorFromAward(indexPath: indexPath))
            }
            .disposed(by: disposeBag)
        
        resultsCollectionView.rx.itemSelected
            .bind { [weak self] indexPath in
                self?.showVotedAnimalDetailPage(votedAnimalDetailReactor: reactor.createVotedDetailReactorFromResult(indexPath: indexPath))
            }
            .disposed(by: disposeBag)
        ...
        reactor.state.map { $0.awardCellReactors.count }
            ...
            .bind { [weak self] count in
                ...
                self?.awardsCollectionView.snp.makeConstraints {
                    $0.height.equalTo(ProfileContestDetailResultAwardCell.Const.cellHeight * CGFloat((count + 1) / 2))
        ...
        reactor.state.map { $0.resultCellReactors.count }
            ...
            .bind { [weak self] count in
                ...
                self?.resultsCollectionView.snp.makeConstraints {
                    $0.height.equalTo(ProfileContestDetailResultVoteCell.Const.cellHeight * CGFloat(count))
                    $0.width.equalTo(DeviceSize.screenWidth)
                }
            }
            .disposed(by: disposeBag)
