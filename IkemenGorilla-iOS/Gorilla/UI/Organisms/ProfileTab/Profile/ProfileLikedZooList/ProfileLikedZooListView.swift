...
final class ProfileLikedZooListView: UICollectionView, View, ViewConstructor {
    ...
    struct Reusable {
        static let zooListCell = ReusableCell<ProfileLikedZooListCell>()
    }
    ...
    // MARK: - Bind Method
    func bind(reactor: ProfileLikedZooListReactor) {
        // Action
        reactor.action.onNext(.load)
        
        // State
        reactor.state.map { $0.likedZooListCellReactors }
